...
const SECTIONS: { h: string; p?: string[]; ul?: string[] }[] = [
  ...
  },
  {
    ...
    ul: [
      ...
      'документы, которые Пользователь направляет для подготовки нотариального действия: их изображения или файлы, включая содержащиеся в них паспортные данные;',
  ...
  },
  {
    ...
    p: [
      'Обработка осуществляется на основании согласия Пользователя, которое запрашивается в форме записи до создания заявки: без согласия заявка не создаётся. Полный текст согласия с перечнем данных, целями и порядком отзыва доступен по ссылке в конце настоящей Политики.',
      'Обработка также осуществляется в случаях, предусмотренных законодательством Российской Федерации, в том числе законодательством о нотариате.',
    ],
  },
  {
  ...
  {
    h: '6. Обработка по поручению Оператора',
    p: [
      'Запись на приём и приём документов выполняет сервис приёма заявок, действующий по поручению Оператора в соответствии с частью 3 статьи 6 Федерального закона № 152-ФЗ. Сервис обрабатывает данные исключительно по указаниям Оператора, собственных целей в них не имеет и обязан соблюдать конфиденциальность.',
      'Базы данных размещены на серверах на территории Российской Федерации. Файлы направленных документов хранятся в зашифрованном виде, каждое обращение к ним фиксируется в журнале доступа, а сами файлы уничтожаются автоматически по истечении установленного срока хранения.',
      'Иным третьим лицам персональные данные не передаются, за исключением случаев, прямо предусмотренных законодательством Российской Федерации.',
    ],
  },
  {
      ...
      <section className="bg-navy-dark">
        <div className="max-w-3xl mx-auto px-4 py-12 md:py-16">
          <div className="space-y-9">
            ...
            {/* Полный текст согласия живёт в сервисе приёма заявок и собирается
                из карточки конторы: там же он и предъявляется Пользователю
                перед созданием заявки. Дублировать его здесь значило бы завести
                вторую редакцию, которая однажды разойдётся с показанной. */}
            <div className="rounded-2xl p-6" style={{ background: '#0f1e35', border: '1px solid rgba(184,154,90,0.15)' }}>
              <h2 className="font-serif text-xl font-bold text-cream mb-3">11. Текст согласия</h2>
              <p className="text-slate leading-relaxed mb-4" style={{ lineHeight: '1.8' }}>
                Полный текст согласия на обработку персональных данных — тот самый, который
                подтверждается при записи, — опубликован в свободном доступе:
              </p>
              <a
                href={`${notarybotUrl}/${notarybotSlug}/privacy`}
                target="_blank"
                rel="noopener"
                className="inline-flex items-center gap-2 text-sm font-semibold text-gold hover:text-gold-light transition-colors no-underline"
              >
