      ...
      <div className="max-w-6xl mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-3 text-sm">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 flex-shrink-0">
            <Image src="/notarius4.png" alt="Нотариус" width={32} height={32} className="object-contain w-8 h-8" />
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-2 md:gap-6 text-center items-center">
          ...
          <a href={notary.phoneHref} className="hover:text-gold transition-colors">{notary.phone}</a>
        </div>
        <div className="flex items-center gap-3">
          <p>© {new Date().getFullYear()} Все права защищены</p>
          <Link href="/admin" className="text-white/10 hover:text-white/30 transition-colors text-xs" title="Управление">🔒</Link>
        </div>
      </div>
    </footer>
  )
