      ...
      <div className="wrap">
        <div className="text-center mb-14 reveal">
          <div className="inline-flex items-center gap-3.5 mb-4">
            ...
            <span className="text-[11px] tracking-[0.32em] uppercase" style={{ color: 'rgb(var(--violet-rgb) / 0.75)' }}>Как всё проходит</span>
            <span className="block w-6 h-px" style={{ background: 'rgb(var(--violet-rgb))' }} />
          </div>
        ...
        <div className="relative mb-9 reveal">
          <span className="absolute left-0 right-0 top-[19px] h-0.5 rounded" style={{ background: 'rgb(var(--violet-rgb) / 0.18)' }} aria-hidden />
          <span
                ...
                <button key={s.t} onClick={() => { setDone(false); setActive(i) }} className="flex flex-col items-center gap-2 group">
                  <span
                    ...
                    style={{
                      ...
                      border: `2px solid ${reached ? 'rgb(var(--violet-rgb))' : 'rgb(var(--violet-rgb) / 0.3)'}`,
                      boxShadow: !done && i === active ? '0 0 0 5px rgb(var(--violet-rgb) / 0.16)' : 'none',
                    }}
                  >
                    {done || i < active ? '✓' : i + 1}
                  </span>
        ...
        {!done && (
          <div key={active} className="dt-card rounded-2xl p-7 sm:p-9 grid sm:grid-cols-[auto_1fr] gap-6 items-start" style={{ background: 'rgb(var(--bg-rgb))', border: '1px solid rgb(var(--violet-rgb) / 0.16)' }}>
            <div className="font-serif leading-none" style={{ fontSize: 'clamp(56px,9vw,84px)', color: 'rgb(var(--violet-rgb) / 0.22)' }}>0{active + 1}</div>
            <div>
              <div className="flex items-center gap-3 mb-3 flex-wrap">
                <h3 className="font-serif m-0" style={{ fontSize: '26px', color: 'rgb(var(--text-rgb))' }}>{cur.t}</h3>
                <span className="text-[11px] font-semibold uppercase tracking-wider rounded-full px-3 py-1" style={{ background: 'rgb(var(--violet-rgb) / 0.12)', color: 'rgb(var(--violet-rgb))' }}>{cur.meta}</span>
              </div>
              ...
              <div className="flex gap-3 mt-6">
                <button
                  ...
                  style={{ color: 'rgb(var(--violet-rgb))', border: '1.5px solid rgb(var(--violet-rgb) / 0.25)' }}
                >← Назад</button>
                <button
        ...
        {/* Финал — путь пройден */}
        {done && (
          <div className="dt-card rounded-2xl p-9 sm:p-12 text-center" style={{ background: 'rgb(var(--bg-rgb))', border: '1px solid rgb(var(--violet-rgb) / 0.16)' }}>
            <div className="w-16 h-16 rounded-full grid place-items-center mx-auto mb-5" style={{ background: 'rgb(var(--violet-rgb))', boxShadow: '0 12px 30px rgb(var(--violet-rgb) / 0.35)' }}>
