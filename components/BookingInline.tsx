...
export default function BookingInline({ initialService, onPick, embedded = false }: Props = {}) {
  ...
  const [previousService, setPreviousService] = useState(initialService)
  if (initialService !== previousService) {
    setPreviousService(initialService)
  ...
  const card = done ? (
            <div className="text-center py-10">
              ...
              <p className="text-slate text-sm">
                ...
                <span className="text-gold font-semibold">{time}</span>. {LIVE_BOOKING ? 'Ожидайте подтверждения конторы.' : 'Запись не отправлена. Данные остались только на этой странице.'}
              </p>
            </div>
          ) : (
            <form onSubmit={submit} className="grid md:grid-cols-2 gap-5">
              ...
              <label className="md:col-span-2 flex items-start gap-2.5 cursor-pointer select-none">
                ...
                <span className="text-[12px] text-slate leading-snug">
                  ...
                  <a href="/consent" target="_blank" rel="noopener noreferrer" className="text-gold underline underline-offset-2">
                    текст согласия
