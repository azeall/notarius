...
const INPUT_STYLE: React.CSSProperties = { background: '#f5ede0', border: '1px solid rgba(192,92,46,0.20)' }
...
export default function AdminAddForm({ defaultStaffId }: { defaultStaffId?: string | null }) {
  ...
  const renderSlot = (t: string) => {
    ...
    if (isStart) { cls = 'text-white font-bold border border-gold'; style = { background: '#c05c2e' } }
    else if (isInSelection && !conflict) { cls = 'text-cream border border-gold/50'; style = { background: 'rgba(192,92,46,0.28)' } }
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl p-4 sm:p-6 mb-8 sm:mb-10" style={{ background: '#fdf8ef', border: '1px solid rgba(192,92,46,0.15)' }}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              style={INPUT_STYLE}
            >
              {SERVICES.map(s => <option key={s} style={{ background: '#f5ede0' }}>{s}</option>)}
            </select>
          </div>
          <div>
            ...
            <div
              ...
              style={
                !selectionFits || selectionConflicts
                  ...
                  : { background: 'rgba(192,92,46,0.10)', border: '1px solid rgba(192,92,46,0.25)', color: '#3d2010' }
              }
            >
