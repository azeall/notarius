    ...
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} width="12" height="12" viewBox="0 0 24 24"
          fill={i < n ? '#1D9E75' : 'none'} stroke="#1D9E75" strokeWidth="2">
...
export default function ReviewsSection() {
  ...
  return (
    <section className="relative py-20 sm:py-[120px]" style={{ background: '#e8f5f0' }}>
      <div className="relative mx-auto px-5 sm:px-8 md:px-10" style={{ maxWidth: '1340px' }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[460px_1fr] gap-8 items-start reveal">
          ...
          <div
            ...
            style={{ border: '1px solid rgba(29,158,117,0.15)', height: '520px', width: '460px', maxWidth: '100%' }}
          >
            {YANDEX_ORG_ID ? (
              <iframe
              ...
              >
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="rgba(29,158,117,0.4)" strokeWidth="1.4">
                  <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/>
                  <circle cx="12" cy="9" r="2.5"/>
                </svg>
                <p className="font-mono text-[11px] tracking-[0.2em] uppercase text-center" style={{ color: 'rgba(29,158,117,0.40)' }}>
          ...
          <div className="flex flex-col gap-5 min-w-0">
                ...
                {([[-1, 'M15 18l-6-6 6-6'], [1, 'M9 18l6-6-6-6']] as [number, string][]).map(([dir, d]) => (
                  <button
                    ...
                    style={{
                      ...
                      color: (dir === -1 ? canLeft : canRight) ? '#1D9E75' : 'rgba(29,158,117,0.25)',
                      background: (dir === -1 ? canLeft : canRight) ? 'rgba(29,158,117,0.08)' : 'transparent',
                    }}
                  >
            ...
            <div className="flex items-center gap-3 pt-3" style={{ borderTop: '1px solid rgba(29,158,117,0.10)' }}>
              ...
              <Stars n={5} />
              <span className="text-[12px]" style={{ color: '#7c8b85' }}>средняя оценка · отзывы клиентов</span>
            </div>
          </div>
        </div>
      </div>
