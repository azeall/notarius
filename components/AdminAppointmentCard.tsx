...
export default function AdminAppointmentCard({ a, isAdmin }: { a: Appointment; isAdmin?: boolean }) {
  ...
  function renderSlot(t: string) {
    ...
    else if (isStart) { cls = 'text-white font-bold border border-gold'; style = { background: '#534AB7' } }
    else if (isInSelection) { cls = 'text-cream border border-gold/50'; style = { background: 'rgba(83,74,183,0.28)' } }
    else if (isOwnOriginal) { cls = 'text-cream/60 border border-dashed'; style = { background: 'rgba(0,0,0,0.04)', borderColor: 'rgba(83,74,183,0.35)' } }
  ...
  if (!editing) {
    return (
      ...
      >
        ...
        {/* Action bar */}
        <div className="flex flex-wrap items-center gap-1.5 mt-3 pt-3" style={{ borderTop: '1px solid rgba(83,74,183,0.10)' }}>
          {/* Contact */}
          <a
          ...
          <span className="mx-0.5 w-px h-4 self-center" style={{ background: 'rgba(83,74,183,0.15)' }} />
        ...
        {isAdmin && reassigning && (
          <div className="flex items-center gap-1.5 mt-2.5 flex-wrap">
            <select
              ...
              style={{ background: '#f4f3fd', border: '1px solid rgba(83,74,183,0.25)' }}
            >
  ...
  // ────────────────────── EDIT ──────────────────────
  return (
    <div className="w-full min-w-0 rounded-2xl px-3 sm:px-6 py-4 sm:py-5" style={{ background: '#ffffff', border: '2px solid rgba(83,74,183,0.40)' }}>
      {/* Header row */}
      ...
      <div className="mb-4">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setDate(d => stepWeekday(d, -1))}
            ...
            style={{ border: '1px solid rgba(83,74,183,0.20)' }} title="Предыдущий рабочий день">‹</button>
          <input
            type="date"
            value={date}
            ...
            style={{ background: '#f4f3fd', border: '1px solid rgba(83,74,183,0.20)', colorScheme: 'light' }}
          />
          <button type="button" onClick={() => setDate(d => stepWeekday(d, +1))}
            className="w-9 h-9 flex items-center justify-center rounded-md text-cream/70 hover:text-gold transition-colors"
            style={{ border: '1px solid rgba(83,74,183,0.20)' }} title="Следующий рабочий день">›</button>
          {date !== a.date && (
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2.5 text-[10px] text-cream/50">
          ...
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: 'rgba(83,74,183,0.28)' }} /> продолжение</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm border border-dashed" style={{ borderColor: 'rgba(83,74,183,0.5)' }} /> исходное</span>
      ...
      <div
        ...
        style={
          !selectionFits || selectionConflicts
            ...
            : changed
              ? { background: 'rgba(83,74,183,0.10)', border: '1px solid rgba(83,74,183,0.3)', color: '#26223d' }
              : { background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(83,74,183,0.15)', color: '#75718f' }
        }
      >
        {!selectionFits
