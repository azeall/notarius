...
export default function AdminAppointmentCard({ a, isAdmin }: { a: Appointment; isAdmin?: boolean }) {
        ...
        {isAdmin && reassigning && (
          <div className="flex items-center gap-1.5 mt-2.5 flex-wrap">
            <select
              ...
              style={{ background: 'rgb(var(--bg-rgb))', border: '1px solid rgba(192,92,46,0.25)' }}
            >
  ...
  // ────────────────────── EDIT ──────────────────────
  return (
    <div className="w-full min-w-0 rounded-2xl px-3 sm:px-6 py-4 sm:py-5" style={{ background: 'rgb(var(--surface-rgb))', border: '2px solid rgba(192,92,46,0.40)' }}>
      {/* Header row */}
      ...
      <div className="mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            style={{ background: 'rgb(var(--bg-rgb))', border: '1px solid rgba(192,92,46,0.20)', colorScheme: 'light' }}
          />
          <button type="button" onClick={() => setDate(d => stepWeekday(d, +1))}
            className="w-9 h-9 flex items-center justify-center rounded-md text-cream/70 hover:text-gold transition-colors"
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
          {DURATION_OPTIONS.map(d => (
            <button key={d} type="button" onClick={() => setDuration(d)}
              ...
              style={duration === d ? { background: 'rgb(var(--violet-rgb))' } : { background: 'rgba(0,0,0,0.03)', borderColor: 'rgba(192,92,46,0.15)' }}>
              {fmtDurLabel(d)}
            </button>
          ))}
        </div>
      ...
      <div
        ...
        style={
          !selectionFits || selectionConflicts
            ...
            : changed
              ? { background: 'rgba(192,92,46,0.10)', border: '1px solid rgba(192,92,46,0.3)', color: 'rgb(var(--text-rgb))' }
              : { background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(192,92,46,0.15)', color: 'rgb(var(--muted-rgb))' }
        }
      >
        {!selectionFits
