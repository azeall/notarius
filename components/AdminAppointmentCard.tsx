...
const STATUS_META: Record<string, { label: string; color: string; bg: string; border: string }> = {
  completed: { label: 'Состоялась', color: 'rgb(var(--ok-2-rgb))', bg: 'rgba(79,157,122,0.14)', border: 'rgba(79,157,122,0.4)' },
...
export default function AdminAppointmentCard({ a, isAdmin }: { a: Appointment; isAdmin?: boolean }) {
  ...
  function renderSlot(t: string) {
    ...
    let cls = 'text-cream/80 border border-[rgb(var(--violet-rgb) / 0.15)] hover:bg-gold/15'
    ...
    else if (isInSelection) { cls = 'text-cream border border-gold/50'; style = { background: 'rgb(var(--violet-rgb) / 0.28)' } }
    else if (isOwnOriginal) { cls = 'text-cream/60 border border-dashed'; style = { background: 'rgba(0,0,0,0.04)', borderColor: 'rgb(var(--violet-rgb) / 0.35)' } }
  ...
  if (!editing) {
    return (
      ...
      >
        ...
        {/* Action bar */}
        <div className="flex flex-wrap items-center gap-1.5 mt-3 pt-3" style={{ borderTop: '1px solid rgb(var(--violet-rgb) / 0.10)' }}>
          {/* Contact */}
          <a
          ...
          <a
            ...
            style={{ color: 'rgb(var(--ok-2-rgb))', border: '1px solid rgba(79,157,122,0.35)' }}
          >
          ...
          <span className="mx-0.5 w-px h-4 self-center" style={{ background: 'rgb(var(--violet-rgb) / 0.15)' }} />
          ...
          {a.status === 'active' ? (
            <>
              <button onClick={() => setStatus('completed')} disabled={busy}
                ...
                style={{ color: 'rgb(var(--ok-2-rgb))', border: '1px solid rgba(79,157,122,0.30)' }}>
                Состоялась
              </button>
        ...
        {isAdmin && reassigning && (
          <div className="flex items-center gap-1.5 mt-2.5 flex-wrap">
            <select
              ...
              style={{ background: 'rgb(var(--bg-rgb))', border: '1px solid rgb(var(--violet-rgb) / 0.25)' }}
            >
  ...
  // ────────────────────── EDIT ──────────────────────
  return (
    <div className="w-full min-w-0 rounded-2xl px-3 sm:px-6 py-4 sm:py-5" style={{ background: 'rgb(var(--surface-rgb))', border: '2px solid rgb(var(--violet-rgb) / 0.40)' }}>
      {/* Header row */}
      ...
      <div className="mb-4">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setDate(d => stepWeekday(d, -1))}
            ...
            style={{ border: '1px solid rgb(var(--violet-rgb) / 0.20)' }} title="Предыдущий рабочий день">‹</button>
          <input
            type="date"
            value={date}
            ...
            style={{ background: 'rgb(var(--bg-rgb))', border: '1px solid rgb(var(--violet-rgb) / 0.20)', colorScheme: 'light' }}
          />
          <button type="button" onClick={() => setDate(d => stepWeekday(d, +1))}
            className="w-9 h-9 flex items-center justify-center rounded-md text-cream/70 hover:text-gold transition-colors"
            style={{ border: '1px solid rgb(var(--violet-rgb) / 0.20)' }} title="Следующий рабочий день">›</button>
          {date !== a.date && (
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
          {DURATION_OPTIONS.map(d => (
            <button key={d} type="button" onClick={() => setDuration(d)}
              ...
              style={duration === d ? { background: 'rgb(var(--violet-rgb))' } : { background: 'rgba(0,0,0,0.03)', borderColor: 'rgb(var(--violet-rgb) / 0.15)' }}>
              {fmtDurLabel(d)}
            </button>
          ))}
        </div>
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2.5 text-[10px] text-cream/50">
          ...
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: 'rgb(var(--violet-rgb) / 0.28)' }} /> продолжение</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm border border-dashed" style={{ borderColor: 'rgb(var(--violet-rgb) / 0.5)' }} /> исходное</span>
      ...
      <div
        ...
        style={
          !selectionFits || selectionConflicts
            ...
            : changed
              ? { background: 'rgb(var(--violet-rgb) / 0.10)', border: '1px solid rgb(var(--violet-rgb) / 0.3)', color: 'rgb(var(--text-rgb))' }
              : { background: 'rgba(0,0,0,0.03)', border: '1px solid rgb(var(--violet-rgb) / 0.15)', color: 'rgb(var(--muted-rgb))' }
        }
      >
        {!selectionFits
