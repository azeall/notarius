        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[13px] tracking-[0.04em]" style={{ color: '#c5bfb0' }}>
            ...
            {status.isOpen && status.closesIn && (
              <span style={{ color: '#7c8b85' }}> · закрывается {status.closesIn}</span>
            )}
            {!status.isOpen && status.opensAt && (
              <span style={{ color: '#7c8b85' }}> · откроется {status.opensAt}</span>
            )}
          </span>
        </div>
        {slots.length > 0 && (
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
            {slots.map((s) => (
              <button
                ...
                style={{ border: '1px solid rgba(29,158,117,0.25)', color: '#c5a84a', background: 'rgba(29,158,117,0.06)' }}
                onMouseEnter={e => { const b = e.currentTarget as HTMLButtonElement; b.style.background = 'rgba(29,158,117,0.14)'; b.style.borderColor = 'rgba(29,158,117,0.5)' }}
                onMouseLeave={e => { const b = e.currentTarget as HTMLButtonElement; b.style.background = 'rgba(29,158,117,0.06)'; b.style.borderColor = 'rgba(29,158,117,0.25)' }}
              >
                {s.label}
              </button>
            ))}
