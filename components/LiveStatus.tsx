        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[13px] tracking-[0.04em]" style={{ color: '#c5bfb0' }}>
            ...
            {status.isOpen && status.closesIn && (
              <span style={{ color: '#94816b' }}> · закрывается {status.closesIn}</span>
            )}
            {!status.isOpen && status.opensAt && (
              <span style={{ color: '#94816b' }}> · откроется {status.opensAt}</span>
            )}
          </span>
        </div>
        {slots.length > 0 && (
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
            {slots.map((s) => (
              <button
                ...
                style={{ border: '1px solid rgba(192,92,46,0.25)', color: '#c5a84a', background: 'rgba(192,92,46,0.06)' }}
                onMouseEnter={e => { const b = e.currentTarget as HTMLButtonElement; b.style.background = 'rgba(192,92,46,0.14)'; b.style.borderColor = 'rgba(192,92,46,0.5)' }}
                onMouseLeave={e => { const b = e.currentTarget as HTMLButtonElement; b.style.background = 'rgba(192,92,46,0.06)'; b.style.borderColor = 'rgba(192,92,46,0.25)' }}
              >
                {s.label}
              </button>
            ))}
